...
use crate::core::wilysearch::traits::{Documents, Indexes, Search, SettingsApi};
use crate::core::wilysearch::types::{AddDocumentsQuery, CreateIndexRequest, SearchRequest, Settings as WilySettings};
...
impl MeilisearchPipeline {
    ...
    async fn extract_to_raw_incremental(
        ...
        while current_page <= total_pages {
            ...
            match meili.add_or_replace_documents(raw_index, &raw_docs, &req) {
                Ok(_) => {
    ...
    pub async fn import_layout_json(
        ...
        meili.add_or_replace_documents(&raw_index, &json_docs, &req).map_err(|e| {
    ...
    pub async fn chunk_from_raw(
        ...
        let query = SearchRequest::default().limit(10000);
        let docs_result = meili.search(raw_index, &query).map_err(|e| {
            SearchError::MeilisearchError(format!("Failed to fetch raw docs: {}", e))
        })?;
        let docs = docs_result.hits;
        ...
        meili.add_or_replace_documents(&chunks_index, &json_chunks, &req).map_err(|e| {
...
fn ensure_raw_index(meili: &Engine, uid: &str) -> Result<(), SearchError> {
    let settings = WilySettings {
        sortable_attributes: Some(vec!["page_number".to_string()]),
        ..Default::default()
    };
    match meili.get_index(uid) {
        Ok(_) => {
            meili.update_settings(uid, &settings).map_err(|e| SearchError::MeilisearchError(e.to_string()))?;
        }
        Err(e) if matches!(e, WilySearchError::IndexNotFound(_)) => {
            ...
            meili.create_index(&req).map_err(|e| SearchError::MeilisearchError(e.to_string()))?;
            meili.update_settings(uid, &settings).map_err(|e| SearchError::MeilisearchError(e.to_string()))?;
        }
        Err(e) => return Err(SearchError::MeilisearchError(e.to_string())),
    }
    Ok(())
}
...
fn ensure_chunks_index(meili: &Engine, uid: &str) -> Result<(), SearchError> {
    let settings = WilySettings {
        searchable_attributes: Some(vec!["content".to_string()]),
        filterable_attributes: Some(vec![
            "source".to_string(),
            "game_system".to_string(),
            "content_category".to_string(),
        ]),
        sortable_attributes: Some(vec!["chunk_index".to_string()]),
        ..Default::default()
    };
    match meili.get_index(uid) {
        Ok(_) => {
...
fn save_library_document(
    ...
    meili
        .add_or_replace_documents(INDEX_LIBRARY_METADATA, &[doc], &req)
...
fn get_highest_page_number(meili: &Engine, raw_index: &str) -> usize {
    let query = SearchRequest::default()
        .sort(vec!["page_number:desc".to_string()])
        .offset(0)
        .limit(1);
    ...
    match meili.search(raw_index, &query) {
        Ok(results) => {
            if let Some(hit) = results.hits.first() {
                if let Some(page_num) = hit.get("page_number") {
...
fn get_content_sample(meili: &Engine, raw_index: &str) -> String {
    let query = SearchRequest::default()
        .sort(vec!["page_number:asc".to_string()])
        .offset(0)
        .limit(20);

    match meili.search(raw_index, &query) {
        Ok(results) => {
            let sample: String = results
                .hits
                .iter()
                .filter_map(|hit| hit.get("raw_content"))
                .filter_map(|c| c.as_str())
                .collect::<Vec<_>>()
                .join(" ");

            // Limit to roughly 10k chars to avoid blowing up the context window
            if sample.len() > 10000 {
                sample.chars().take(10000).collect()
            } else {
                sample
            }
        }
