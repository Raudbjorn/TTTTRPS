...
//! batch pre-generation, cancellation support, and event emission.
...
pub use events::{channels, emit_event, QueueEventEmitter, JobStatusEvent, JobSubmittedEvent, QueueStatsEvent};
pub use types::{
...
/// Voice synthesis queue with priority handling, progress tracking, and event emission
...
impl SynthesisQueue {
    ...
    pub async fn submit(
        ...
        emitter: Option<&dyn QueueEventEmitter>,
    ) -> QueueResult<String> {
        ...
        // Emit event
        emit_event(emitter, channels::JOB_SUBMITTED, &JobSubmittedEvent {
            job_id: job_id.clone(),
    ...
    pub async fn submit_batch(
        ...
        for job in jobs {
            let id = self.submit(job, emitter).await?;
            ids.push(id);
        }
    ...
    /// Cancel a job
    pub async fn cancel(&self, job_id: &str, emitter: Option<&dyn QueueEventEmitter>) -> QueueResult<()> {
        ...
        // Emit event
        emit_event(emitter, channels::JOB_CANCELED, &JobStatusEvent {
    ...
    pub async fn cancel_all(&self, emitter: Option<&dyn QueueEventEmitter>) -> QueueResult<usize> {
        let mut state = self.state.write().await;
        let mut canceled = 0;
        ...
        // Emit stats event
        emit_event(emitter, channels::QUEUE_STATS, &QueueStatsEvent {
    ...
    pub async fn pause(&self, emitter: Option<&dyn QueueEventEmitter>) {
        let mut state = self.state.write().await;
        state.is_paused = true;

        emit_event(emitter, channels::QUEUE_PAUSED, &());

        log::info!("Synthesis queue paused");
    }

    /// Resume queue processing
    pub async fn resume(&self, emitter: Option<&dyn QueueEventEmitter>) {
        let mut state = self.state.write().await;
        state.is_paused = false;

        emit_event(emitter, channels::QUEUE_RESUMED, &());
    ...
    pub async fn mark_started(
        ...
        // Emit event
        emit_event(emitter, channels::JOB_STARTED, &JobStatusEvent {
            job_id: job_id.to_string(),
    ...
    pub async fn update_progress(
        ...
        // Emit event
        emit_event(emitter, channels::JOB_PROGRESS, &JobStatusEvent {
            job_id: job_id.to_string(),
    ...
    pub async fn mark_completed(
        ...
        // Emit event
        emit_event(emitter, channels::JOB_COMPLETED, &JobStatusEvent {
            job_id: job_id.to_string(),
    ...
    pub async fn mark_failed(
        ...
        if should_retry {
            ...
            emit_event(emitter, channels::JOB_FAILED, &JobStatusEvent {
                job_id: job_id.to_string(),
