...
impl<S: TokenStorage + 'static> ClaudeClient<S> {
    ...
    pub async fn get_token_info(&self) -> Result<Option<TokenInfo>> {
        let res = self.oauth.read().await.storage().load(PROVIDER_ID).await?;
    ...
    async fn get_access_token(&self) -> Result<String> {
        ...
        if let Some(token_info) = oauth.storage().load(PROVIDER_ID).await? {
    ...
    pub async fn collect_text(mut self) -> Result<String> {
        ...
        while let Some(event) = self.next().await {
            if let StreamEvent::ContentBlockDelta { delta, .. } = event? {
                if let crate::oauth::claude::models::ContentDelta::TextDelta { text: t } = delta {
                    text.push_str(&t);
                }
            }
        }
