...
///
/// ```rust,ignore
/// use crate::oauth::gemini::convert::convert_request;
/// use crate::oauth::gemini::MessagesRequest;
///
        ...
        if !tools.is_empty() {
            ...
            if is_claude {
                google_request.tool_config = Some(ToolConfig {
                    function_calling_config: crate::oauth::gemini::models::google::FunctionCallingConfig {
...
fn convert_system_prompt(system: &SystemPrompt) -> Vec<Part> {
    match system {
        ...
        SystemPrompt::Blocks(blocks) => blocks
            .iter()
            .filter_map(|block| {
                let crate::oauth::gemini::models::request::SystemBlock::Text { text, .. } = block;
                if text.trim().is_empty() {
                    None
                } else {
                    Some(Part::text(text))
...
fn convert_tools(tools: &[crate::oauth::gemini::models::tools::Tool]) -> Vec<FunctionDeclaration> {
    tools
        .iter()
        .enumerate()
        .map(|(idx, tool)| {
...
fn convert_tool_choice(choice: &crate::oauth::gemini::models::tools::ToolChoice) -> ToolConfig {
    match choice {
        crate::oauth::gemini::models::tools::ToolChoice::Auto => ToolConfig::auto(),
        crate::oauth::gemini::models::tools::ToolChoice::Any => ToolConfig::any(),
        crate::oauth::gemini::models::tools::ToolChoice::None => ToolConfig::none(),
        crate::oauth::gemini::models::tools::ToolChoice::Tool { name } => ToolConfig::force(name),
    }
}
...
mod tests {
    use super::*;
    use crate::oauth::gemini::models::tools::Tool;
    use serde_json::json;
    ...
    fn test_convert_tool_choice_auto() {
        let config = convert_tool_choice(&crate::oauth::gemini::models::tools::ToolChoice::Auto);
        assert_eq!(config.function_calling_config.mode, "AUTO");
    }

    #[test]
    fn test_convert_tool_choice_any() {
        let config = convert_tool_choice(&crate::oauth::gemini::models::tools::ToolChoice::Any);
        assert_eq!(config.function_calling_config.mode, "ANY");
    }

    #[test]
    fn test_convert_tool_choice_none() {
        let config = convert_tool_choice(&crate::oauth::gemini::models::tools::ToolChoice::None);
        assert_eq!(config.function_calling_config.mode, "NONE");
    }

    #[test]
    fn test_convert_tool_choice_specific() {
        let config = convert_tool_choice(&crate::oauth::gemini::models::tools::ToolChoice::tool("my_tool"));
        assert_eq!(config.function_calling_config.mode, "ANY");
        assert_eq!(
    ...
    fn test_convert_system_blocks() {
        use crate::oauth::gemini::models::request::SystemBlock;
