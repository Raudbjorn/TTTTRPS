...
impl CharacterGenState {
    ...
    fn render_system_select(&self, frame: &mut Frame, area: Rect) {
        ...
        for (i, sys) in self.systems.iter().enumerate() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(sys.description.clone(), Style::default().fg(theme::TEXT_MUTED)),
            ]));
        }
    ...
    fn render_display(&self, frame: &mut Frame, area: Rect) {
        ...
        if let Some(ref race) = character.race {
            info_parts.push(Span::styled(race.clone(), Style::default().fg(theme::PRIMARY_LIGHT)));
        ...
        if !bg.origin.is_empty() || !bg.motivation.is_empty() {
            ...
            if !bg.motivation.is_empty() {
                lines.push(Line::from(vec![
                    Span::raw("  "),
                    Span::styled("Motivation: ", Style::default().fg(theme::TEXT_MUTED)),
        ...
        lines.push(Line::from(vec![
            Span::raw("  "),
            Span::styled("b", Style::default().fg(theme::TEXT_MUTED)),
            Span::raw(":backstory "),
    ...
    fn render_saved_list(&self, frame: &mut Frame, area: Rect) {
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled("j/k", Style::default().fg(theme::TEXT_MUTED)),
            Span::raw(":navigate "),
            Span::styled("l/Esc", Style::default().fg(theme::TEXT_MUTED)),
            Span::raw(":close"),
        ]));
...
mod tests {
    ...
    fn test_gen_phase_transitions() {
        let mut state = CharacterGenState::new();
    ...
    fn test_gen_form_to_options() {
        let state = CharacterGenState::new();
        if state.systems.is_empty() {
            return;
        }
