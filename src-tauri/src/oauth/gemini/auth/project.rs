...
//!
//! ```rust,ignore
//! use crate::oauth::gemini::auth::project::{discover_project, ProjectInfo, SubscriptionTier};
//!
...
use crate::oauth::gemini::error::{AuthError, Error, Result};
...
impl std::str::FromStr for SubscriptionTier {
    ...
    ///
    /// ```
    /// use crate::oauth::gemini::auth::project::SubscriptionTier;
    /// use std::str::FromStr;
    ///
...
///
/// ```rust,ignore
/// use crate::oauth::gemini::auth::project::discover_project;
///
/// # async fn example() -> crate::oauth::gemini::Result<()> {
...
pub async fn discover_project(token: &str, hint_project_id: Option<&str>) -> Result<ProjectInfo> {
    ...
    if let Some(e) = last_error {
        warn!(error = %e, "All endpoints failed to discover project, falling back to default project ID: {}", DEFAULT_PROJECT_ID);
        tracing::error!("PROJECT DISCOVERY FAILED: The fallback project ID '{}' may not be accessible to this user", DEFAULT_PROJECT_ID);
        return Ok(ProjectInfo::default_fallback());
    }
...
///
/// ```rust,ignore
/// use crate::oauth::gemini::auth::project::onboard_user;
///
/// # async fn example() -> crate::oauth::gemini::Result<()> {
