...
//!
//! ```rust,ignore
//! use crate::oauth::gemini::{CloudCodeClient, FileTokenStorage, MessagesRequest};
//!
//! # async fn example() -> crate::oauth::gemini::Result<()> {
...
impl<S: TokenStorage + 'static> CloudCodeClient<S> {
    ...
    ///
    /// The project ID is cached in memory and persisted in the token storage
    /// using the composite token format. On first call:
    /// 1. Checks in-memory cache
    /// 2. Checks token storage for embedded project ID
    /// 3. Discovers project via API if not found
    /// 4. Persists discovered project ID back to token storage
    ///
    /// Uses double-check pattern to avoid redundant discovery when multiple tasks race.
    ...
    pub async fn get_project_id(&self) -> Result<String> {
        ...
        // Check if token has stored project ID (persisted from previous session)
        if let Ok(Some(token)) = self.oauth.storage().load("gemini").await {
            if let Some(project_id) = token.project_id() {
                debug!(project_id = %project_id, "Loaded project ID from stored token");

                // Cache in memory
                {
                    let mut cached = self.project_id.write().await;
                    if cached.is_none() {
                        *cached = Some(project_id.clone());
                    }
                }

                if let Some(managed) = token.managed_project_id() {
                    let mut managed_cached = self.managed_project_id.write().await;
                    if managed_cached.is_none() {
                        *managed_cached = Some(managed);
                    }
                }
        ...
        let project_info = discover_project(&access_token, None).await?;
        ...
        // Persist project ID to token storage for future sessions
        if let Ok(Some(current_token)) = self.oauth.storage().load("gemini").await {
            let updated_token = current_token.with_project_ids(
                &project_info.project_id,
                project_info.managed_project_id.as_deref(),
            );
            if let Err(e) = self.oauth.storage().save("gemini", &updated_token).await {
                warn!(error = %e, "Failed to persist project ID to token storage");
            } else {
                debug!(project_id = %project_info.project_id, "Persisted project ID to token storage");
            }
        }
...
///
/// ```rust,ignore
/// use crate::oauth::gemini::{CloudCodeClient, FileTokenStorage};
