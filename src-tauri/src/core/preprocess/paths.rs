...
/// 1. Bundled resource path (production builds via Tauri)
/// 2. Development path (`src-tauri/data/`)
/// 3. User data directory fallback
///
/// Returns `None` if the dictionary cannot be found.
pub fn get_english_dictionary_path(app_handle: Option<&tauri::AppHandle>) -> Option<PathBuf> {
    get_dictionary_path(ENGLISH_DICT_FILENAME, app_handle)
}
...
/// 1. Tauri resource path (bundled in production)
/// 2. Development source tree
/// 3. User data directory
fn get_dictionary_path(filename: &str, app_handle: Option<&tauri::AppHandle>) -> Option<PathBuf> {
    // 1. Try Tauri bundled resource path (production builds)
    if let Some(handle) = app_handle {
        if let Ok(resource_dir) = handle.path().resource_dir() {
            let bundled_path = resource_dir.join("data").join(filename);
            if bundled_path.exists() {
                return Some(bundled_path);
            }
            // Also try without data/ prefix (depends on how resources are bundled)
            let flat_path = resource_dir.join(filename);
            if flat_path.exists() {
    ...
    let dev_paths = [
        ...
        PathBuf::from("src-tauri/data").join(filename),
...
mod tests {
    ...
    fn test_dev_path_resolution() {
        // In development, should find the dictionary in src-tauri/data/
        // This test will pass when run from the correct directory
