...
use super::events::{channels, QueueStatsEvent};
...
impl QueueWorker {
    ...
    async fn process_next(&self) {
        ...
        let _ = self
            .queue
            .update_progress(&job_id, 0.1, "Connecting to provider", Some(&self.app_handle))
            .await;
        ...
        let _ = self.app_handle.emit(channels::QUEUE_STATS, QueueStatsEvent { stats });
    }
}
